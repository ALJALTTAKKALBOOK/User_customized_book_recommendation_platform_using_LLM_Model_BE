...
class Book(Base):
    ...
    grade_point: Mapped[int] = mapped_column(Integer)        # 평점 (예: 1~100)
    page_count: Mapped[int] = mapped_column(Integer)         # 쪽수
    cover_url: Mapped[str] = mapped_column(String(500))      # 이미지 링크
    total_readers: Mapped[int] = mapped_column(BigInteger)   # 누적 독자수
    published_at: Mapped[datetime] = mapped_column(DateTime(timezone=True)) # 출판일시
